...
namespace _1195 {
    ...
    */
    ...
    void FizzBuzz::fizz(std::function<void()> printFizz) {
        run([this]() { return index % 3 == 0 && index % 5 != 0; }, [&]() { printFizz(); output << "fizz"; });
    }

    void FizzBuzz::buzz(std::function<void()> printBuzz) {
        run([this]() { return index % 3 != 0 && index % 5 == 0; }, [&]() { printBuzz(); output << "buzz"; });
    }

    void FizzBuzz::fizzbuzz(std::function<void()> printFizzBuzz) {
        run([this]() { return index % 3 == 0 && index % 5 == 0; }, [&]() { printFizzBuzz(); output << "fizzbuzz"; });
    }

    void FizzBuzz::number(std::function<void(int)> printNumber) {
        run([this]() { return index % 3 != 0 && index % 5 != 0; }, [&]() { printNumber(index); output << std::to_string(index); });
    }
