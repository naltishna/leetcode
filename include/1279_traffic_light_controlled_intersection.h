...
namespace _1279 {
    ...
    public:
        ...
        void carArrived(
            int carId, // the id of the car
            int roadId, // the id of the road that the car travels (1 or 2) == ('A' or 'B' on a screen)
            int direction, // the direction of the car
            std::function<void()> turnGreen, // a function to turn the traffic light to green on the current road
            std::function<void()> crossCar // a function to let the current car cross the intersection
        );
    };
